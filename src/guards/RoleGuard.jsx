...
const CSS = `
  ...
  .pr-icon  { font-size: 52px; margin-bottom: 20px; display: block; line-height: 1; }
  .pr-title { font-family: 'Syne', sans-serif; font-size: 22px; font-weight: 800; color: #fff; margin-bottom: 10px; }
  .pr-sub   { font-size: 14px; color: rgba(255,255,255,.4); line-height: 1.7; margin-bottom: 28px; }

  .pr-steps { display: flex; flex-direction: column; gap: 10px; margin-bottom: 28px; text-align: left; }
  .pr-step  { display: flex; align-items: flex-start; gap: 12px; background: rgba(255,255,255,.03); border: 1px solid rgba(255,255,255,.06); border-radius: 10px; padding: 12px 14px; }
  .pr-step-num { width: 22px; height: 22px; border-radius: 50%; background: rgba(245,158,11,.12); border: 1px solid rgba(245,158,11,.25); display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 800; color: #f59e0b; flex-shrink: 0; margin-top: 1px; }
  ...
  .pr-eta { display: inline-flex; align-items: center; gap: 7px; background: rgba(245,158,11,.07); border: 1px solid rgba(245,158,11,.18); border-radius: 20px; padding: 7px 16px; font-size: 12px; font-weight: 600; color: #f59e0b; margin-bottom: 28px; }

  .pr-btn { display: block; width: 100%; padding: 12px; border-radius: 10px; font-size: 14px; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; transition: all .2s; text-decoration: none; text-align: center; border: none; }
  .pr-btn.outline  { background: transparent; border: 1px solid rgba(255,255,255,.1); color: rgba(255,255,255,.4); margin-top: 10px; }
  .pr-btn.outline:hover { border-color: rgba(239,68,68,.3); color: rgba(239,68,68,.6); }
  .pr-btn.primary  { background: linear-gradient(45deg, #00f3ff, #0066ff); color: #000; }
  .pr-btn.primary:hover { opacity: .85; }
  .pr-btn.danger   { background: rgba(239,68,68,.08); border: 1px solid rgba(239,68,68,.2); color: #ef4444; }
  .pr-btn.danger:hover { background: rgba(239,68,68,.15); }

  .pr-pulse-wrap { position: relative; width: 72px; height: 72px; margin: 0 auto 24px; }
  .pr-pulse-ring { position: absolute; inset: 0; border-radius: 50%; border: 2px solid rgba(245,158,11,.3); animation: prPulse 2s ease-out infinite; }
  .pr-pulse-ring:nth-child(2) { animation-delay: .6s; }
  .pr-pulse-ring:nth-child(3) { animation-delay: 1.2s; }
  .pr-pulse-core { position: absolute; inset: 16px; border-radius: 50%; background: rgba(245,158,11,.12); border: 1px solid rgba(245,158,11,.3); display: flex; align-items: center; justify-content: center; font-size: 22px; }
  @keyframes prPulse { 0% { transform: scale(1); opacity: .6; } 100% { transform: scale(1.6); opacity: 0; } }
...
function PantallaPendiente({ user, onRefresh, onLogout }) {
  ...
  const handleRefresh = async () => {
    ...
    await onRefresh()
    setChecking(false)
    console.log("estado después del refresh:", user?.estado)
  }
